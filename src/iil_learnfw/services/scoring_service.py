...
def submit_answer(
    ...
    if selected_answer_id and question.question_type in ("single_choice", "multiple_choice"):
...
def finish_attempt(attempt_id: int) -> Attempt:
    ...
    awarded_points = sum(a.points_awarded for a in attempt.answers.all())
    ...
    logger.info("Attempt %d finished: %s (%.1f%%)", attempt_id, status, score)
    return attempt
...
def can_retry(user, quiz_id: int) -> bool:
    ...
    if quiz.max_attempts == 0:
        return True
    attempt_count = Attempt.objects.filter(user=user, quiz=quiz, completed_at__isnull=False).count()
