...
class AssessmentService:
    ...
    def start_attempt(
        *,
        ...
        try:
            ...
            raise AssessmentValidationError(f"Assessment-Typ '{assessment_type_slug}' nicht gefunden.")

        if not assessment_type.is_active:
            raise AssessmentValidationError(f"Assessment-Typ '{assessment_type_slug}' ist nicht aktiv.")

        if not assessment_type.is_public and user is None:
            raise AssessmentValidationError("Dieses Assessment erfordert einen eingeloggten Benutzer.")
    ...
    def submit_attempt(
        *,
        ...
        try:
            ...
            raise AssessmentValidationError(f"Attempt '{attempt_public_id}' nicht gefunden.")
        ...
        if not questions:
            raise AssessmentValidationError(f"Assessment-Typ '{at.key}' hat keine aktiven Fragen.")
    ...
    def get_result(
        *,
        ...
        return {
            ...
            "has_certificate": (hasattr(attempt, "report") and attempt.report is not None and attempt.report.certificate_id is not None),
        }
        ...
        *,
        ...
        for i, current in enumerate(levels):
            for other in levels[i + 1 :]:
                if current.pct_min <= other.pct_min <= current.pct_max:
                    errors.append(f"Überlappung: '{current.key}' ({current.pct_min}-{current.pct_max}%) und '{other.key}' ({other.pct_min}-{other.pct_max}%)")
        ...
        if levels and not errors:
            ...
            if sorted_levels[0].pct_min > 0:
                errors.append(f"Lücke: 0-{sorted_levels[0].pct_min - 1}% ist keinem Reifegrad zugeordnet.")
            for i in range(len(sorted_levels) - 1):
                gap_start = sorted_levels[i].pct_max + 1
                gap_end = sorted_levels[i + 1].pct_min - 1
                if gap_start <= gap_end:
                    errors.append(f"Lücke: {gap_start}-{gap_end}% ist keinem Reifegrad zugeordnet.")
            if sorted_levels[-1].pct_max < 100:
                errors.append(f"Lücke: {sorted_levels[-1].pct_max + 1}-100% ist keinem Reifegrad zugeordnet.")
        return errors
    ...
    def _hash_ip(ip_address: str) -> str:
        ...
        if not salt:
            logger.warning("ASSESSMENT_IP_HASH_SALT nicht konfiguriert — IP-Hashing ohne Salt unsicher.")
