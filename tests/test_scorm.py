...
@pytest.fixture
def course(db):
    return Course.objects.create(title="SCORM Course", slug="scorm-course", status="published")
...
class TestScormPackage:
    ...
    def test_should_support_2004(self, course):
        pkg = ScormPackage.objects.create(course=course, scorm_version="2004")
...
class TestScormTracking:
    def test_should_create_tracking(self, user, package):
        tracking = ScormTracking.objects.create(user=user, package=package, status="incomplete")
...
class TestCourseManager:
    ...
    def test_should_return_tenant_and_global(self, global_course, tenant_course):
        tc, tid = tenant_course
        qs = Course.objects.for_tenant(tid)
        assert global_course in qs
        assert tc in qs

    def test_should_exclude_other_tenant(self, global_course, tenant_course):
        _, tid = tenant_course
    ...
    def test_should_combine_tenant_and_published(self, global_course, tenant_course):
        tc, tid = tenant_course
